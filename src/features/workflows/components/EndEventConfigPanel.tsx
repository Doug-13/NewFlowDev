...
export function EndEventConfigPanel({
  ...
  const syncMetadataState = (nextSetIds: string[], nextManualIds: string[], nextFields?: ActivityMetadataFieldRule[]) => {
    ...
    form.setFieldsValue({
      ...
      finalMetadataDefinitionIds: effectiveIds,
  ...
  // ── Inicialização ──────────────────────────────────────────────────────────
  useEffect(() => {
    if (!selectedElement || selectedElement.kind !== 'end') return
    ...
    const cfg = initialConfig?.kind === 'end' ? (initialConfig.config as EndEventConfig) : null

    const initialMetadataFields =
      cfg?.metadataFields && cfg.metadataFields.length > 0
        ? cfg.metadataFields
        : (cfg?.finalMetadataDefinitionIds ?? []).map((id) =>
    ...
    form.setFieldsValue({
      finalMetadataDefinitionIds: initialDefinitionIds,
      metadataSetIds:             initialSetIds,
      metadataFields:             initialMetadataFields,
      summarySections:            cfg?.summarySections    ?? [],
      notificationTemplateIds:    cfg?.notificationTemplateIds ?? [],
      finalAction:                cfg?.finalAction        ?? 'complete',
  ...
  // ── Guard ──────────────────────────────────────────────────────────────────
  if (!selectedElement || selectedElement.kind !== 'end') {
    return <Empty description="Selecione um evento final no fluxo" style={{ padding: 32 }} />
  }
  ...
  const handleSubmit = (values: FormValues) => {
    ...
    onSave({
      ...
      kind: 'end',
      config: {
        finalMetadataDefinitionIds: normalizedFields.map((f) => f.metadataDefinitionId),
    ...
    <Form<FormValues> form={form} layout="vertical" onFinish={handleSubmit}>
      <Tabs
        ...
        items={[
          ...
          {
            key: 'config',
            label: <Space size={6}><StopOutlined /><span>Configuração</span></Space>,
            children: (
              <div style={tabPaneStyle}>
                <Text style={sectionLabelStyle}>Ação final</Text>
                <Form.Item name="finalAction" label="Ação ao encerrar" style={{ marginBottom: 16 }}>
                  <Select
                    options={[
                ...
                {finalAction === 'open-linked-workflow' && (
                  <Form.Item label="Workflow vinculado" name="linkedWorkflowId" style={{ marginBottom: 16 }}>
                    <Input placeholder="Informe o ID do workflow vinculado" />
                  </Form.Item>
                )}

                <Text style={sectionLabelStyle}>Resumo final</Text>
                <Form.Item label="Seções do resumo" name="summarySections" style={{ marginBottom: 16 }}>
                  <Select
                    mode="tags"
                ...
                <Form.Item label="Instruções finais" name="instructions" style={{ marginBottom: 0 }}>
                  <Input.TextArea
                    rows={4}
          ...
          // ── ABA: METADADOS ─────────────────────────────────────────────────
          {
            ...
            children: (
              <div style={tabPaneStyle}>
                ...
                <Text style={sectionLabelStyle}>Metadados do evento final</Text>
                ...
                <Form.Item label="Metadados selecionados" style={{ marginBottom: 16 }}>
                  <Select
                    ...
                    placeholder={metadataDefinitionsLoading ? 'Carregando metadados...' : 'Selecione os metadados de encerramento'}
                ...
                {resolvedMetadataFields.length === 0 ? (
                  <Alert
                    ...
                    description="Selecione conjuntos ou metadados específicos para exibir no encerramento."
                ...
                <Text type="secondary" style={{ display: 'block', marginTop: 10, fontSize: 12 }}>
                  "Somente leitura" deixa o campo visível no encerramento, mas sem permitir edição.
                </Text>
              </div>
            ),
          },
          ...
          // ── ABA: NOTIFICAÇÕES ──────────────────────────────────────────────
          {
            ...
            children: (
              <div style={tabPaneStyle}>
                ...
                <Form.Item
                  label="Notificações disparadas ao encerrar"
      ...
      <Form.Item name="finalMetadataDefinitionIds" hidden><Input /></Form.Item>
